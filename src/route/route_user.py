...
from src.repositories.refresh_token_repo import (
    delete_refresh_token,
    refresh_token_create_by_bd,
)
...
async def create_user(user: CreateUserScheme, session: SessionDep):
    logger.info(f"Creating new user with: {user}")
...
async def user_join_team(
    ...
    return {"message": f"Joined team {result.name}"}
...
async def put_user_me(
    ...
    result = await crud_u.update_user(
        session=session, current_user=user, update_data=update_data
    )
    return result
...
async def delete_user_me(
    user: Annotated[User, Depends(jwt.get_current_user)], session: SessionDep
):
