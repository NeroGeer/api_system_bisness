...
async def require_admin_or_team_manager(
    ...
    team_exists = await session.scalar(select(exists().where(Team.id == team_id)))
    if not team_exists:
        raise HTTPException(status_code=404, detail="Team not found")

    check_roles: set[TeamRole] = (
        team_role if team_role is not None else {TeamRole.manager}
    )

    is_admin = has_role(user=current_user, roles={UserRole.admin})
    is_team_role = await has_team_role(
        session=session, user=current_user, team_id=team_id, roles=check_roles
    )
    is_executor = False
