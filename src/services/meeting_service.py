...
async def validate_meeting_data(
    ...
    logger.debug(
        f"Validating meeting data: team_id={team_id}, creator_id={creator.id}"
    )
    ...
    if missing_users:
        logger.warning(
            f"Missing users in system: {missing_users}"
        )
    ...
    if invalid_users:
        logger.warning(
            f"Users not in team: team_id={team_id}, users={invalid_users}"
        )
        raise HTTPException(status_code=400, detail=f"Users not in team: {invalid_users}")

    if start_time and end_time:
        logger.debug(
            f"Checking meeting conflicts: users={participants_set}"
        )
